...
/**
 * Class Corte de Servicio de Televisión
 *
 * Esta clase permite realizar el proceso de corte de Servicio para Minerva y VO
 *
 *                                  ╔═╗      ╔═╗
 *                                ╔═╝ ╚═╗    ║ ║
 *                          ╔════╗╚╗  ╔═╝ ╔══╝ ║
...
class TvCorteServicio
{
     ...
     * corteServicio
     *
     * Esta función genera el Corte de servicio para las plataformas definidas
     *
     * @return struct
     */
    public function corteServicio()
    {
        $return = [];
        ...
        $this->_util->setLog(
            ...
            "MEDIADOR",
            "Inicia proceso corte de servicio en Minerva y Vo",
            "INICIO",
        );
        ...
        if ($validaDb && $validaDbMinerva) {
            ...
            if ($validaParametros === TRUE) {
                // Se invoca el proceso de corte para un servicio en Minerva
                // un servicio en Minerva
                $return = $this->_corteServicioMinerva();
                ...
                // Se invoca el proceso de corte para un servicio en VO
                $return = $this->_corteServicioVO();
                $this->_return["default"] = $return["default"];
                ...
                if ($this->_return["default"]["status"] !== FALSE) {
                    ...
                    $this->_util->setLog(
                        ...
                        "C O R T E   D E   S E R V I C I O",
                        "MEDIADOR",
                        "Finaliza proceso de corte servicio en Minerva y Vo",
                        "FIN",
                    );
                }
            }
    ...
    /**
     * _corteServicioMinerva
     *
     * Esta función genera el Corte de servicio en la plataforma Minerma
     *
     * @return void
     */
    private function _corteServicioMinerva()
    {
        $return = [];
        $ws = new Ws();
     ...
     * _corteServicioVo
     *
     * Esta función genera el Corte de servicio en la plataforma VO
     *
     * @return strunct
     */
    private function _corteServicioVo()
    {
        $return = [];
        $vo = new Vo();
        ...
        if ($rsPlataforma === TRUE) {
            ...
            if ($rsVAlidaUrl === TRUE) {
                ...
                if($rsToken === TRUE) {
                    // Prepara los datos para corte de VO
                    $validaPrepara = $this->_prepararDatosVo();
                    if ($validaPrepara) {
                        // Procesa Corte servicio VO
                        $return = $this->_setCorteServicioVo(
                            $vo,
                            $this->_aux
                        );
                    }
    ...
    /**
     * setCorteServicioMinerva
     *
     * @param  mixed $ws
     * @param  mixed $info
     * @return void
     */
    public function setCorteServicioMinerva($ws, $info)
    {
        $return         = [];
        ...
        if($rsPlanes !== -1) {
            // Consumo del servicio de Corte de Minerva
            $result = $ws->consumeWs(
                "CORTE_SERVICIO",
                "TraeRegistros",
                $parametros
            ...
            if ($result) {
                error_log("Corte en Minerva");
                if (is_array($result)) {
                    error_log("conexion corte_servicio");
                    if ($result[0]["estado"] == 0
                     || $result[0]["estado"] == 1002) {
                        $estado = "CORTE DE SERVICIO CORRECTO";
                        ...
                        $this->_util->setLog(
                            $this->_config->urlLog,
                            "Corte Servicio Minerva:OK",
                            "MEDIADOR"
                        );
                    } else {
                        $estado = "WS CORTE ESTADO DE RESPUESTA NO ES CORRECTO";
                        // informacion para construir log de respuesta y error
                        $data = [
                            "messageLog"    => "ERROR: WS CORTE ESTADO NO OK",
                            "type"          => "ESTADO",
                            "platform"      => "MINERVA",
                            "state"         => FALSE,
                            "detail"        => "",
                    ...
                    error_log("Resultado de corte_servicio" . $estado);
                } else {
                    $estado = "RESPUESTA DE WS CORTE NO ES CORRECTA";
                    // informacion para construir log de respuesta y error
                    $data = [
                        "messageLog"    => "ERROR: WS CORTE RESPONDE INCORRECTO",
                        "type"          => "RESPUESTA",
                        "platform"      => "MINERVA",
                        "state"         => FALSE,
                        "detail"        => "",
                ...
                // informacion para construir log de respuesta y error
                $data = [
                    "messageLog"    => "ERROR: FALLO CONSUMO WS CORTE SERVICIO",
                    "type"          => "CONEXION",
                    "platform"      => "MINERVA",
                    "state"         => FALSE,
                    "detail"        => "",
     ...
     * @return void
     */
    private function _setCorteServicioVo($vo, $info)
    {
        $return   = [];
        $objSql = $this->_sqlMediador;
        ...
        $rsServicio = $vo->suspendServicio($info);
        if ($rsServicio === TRUE) {
            //Respuesta OK
            $estado = "CORTE DE SERVICIO CORRECTO";
            // informacion para construir log de respuesta y error
            $data = [
                ...
                "output"     => $estado,
                "ws"         => "api/service/{servicecode}/suspend",
                "result"     => [
                    "estado" => $estado,
                    "estado_desc" => "OK"
                ]
            ...
            $estado = "NO SE PUDO CORTAR EL SERVICIO [ " . $responseApi["message"] . " ]";
            // informacion para construir log de respuesta y error
            $data = [
                "messageLog"    => "ERROR: NO SE PUDO CORTAR EL SERVICIO [ " . $responseApi["message"] . " ]",
                "type"          => "SERVICIO",
                "platform"      => "VO",
                "state"         => FALSE,
                "detail"        => "",
