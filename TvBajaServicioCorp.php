...
class TvBajaServicioCorp
{
    ...
    public function bajaServicio()
    {
        ...
        $this->_util->setLog(
            $this->_config->urlLog,
            "B A J A   D E   S E R V I C I O   C O R P",
            "MEDIADOR",
    ...
    public function setBajaServicioMinerva($ws, $info)
    {
        ...
        $rsPlanes = $ws->getPlanesMinervaByCustomerId($instancia);
        if($rsPlanes !== -1) {
            // Consumo del servicio de Baja de servicio de Minerva
            $result = $ws->consumeWs(
                "BAJA_SERVICIO",
                "TraeRegistros",
                $parametros
    ...
    private function _setBajaServicioVo($vo, $info)
    {
        ...
        if ($rsServicio === TRUE) {
            ...
            $estado = "NO SE PUDO ELIMINAR EL SERVICIO";
            // informacion para construir log de respuesta y error
            $data = [
                "messageLog"    => "ERROR: NO SE PUDO ELIMINAR EL SERVICIO",
                "type"          => "SERVICIO",
                "platform"      => "VO",
                "state"         => FALSE,
                "detail"        => "",
