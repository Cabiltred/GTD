...
class TvAltaServicio
{
    ...
    private function _prepararDatosVo()
    {
        ...
        $puerto .= $vecServId[5] . "-";
        $puerto .= $vecServId[6];
        $this->_aux["vo"]["ftthPort"]    = $puerto;

        // Prepara Nodo
        $nodo  = $vecServId[1] . "-";
        $nodo .= $vecServId[2];
        $this->_aux["vo"]["ftthNode"]    = $nodo;
            ...
            if (is_array($result)) {
                ...
                if ($result[0]["estado"] == 0 || $result[0]["estado"] == 1002) {
                    ...
                    if ($exitosM == count($macs)) {
                        ...
                        $estado = "WS ALTA NO SE ASIGNARON DECOS";
                        // informacion para construir log de respuesta y error
                        $data = [
                            "messageLog"    => "ERROR: WS ALTA NO ASIGNA DECOS",
                            "type"          => "DECO",
                            "platform"      => "MINERVA",
                            "state"         => FALSE,
                            "detail"        => "",
            ...
            if ($rsCliente === TRUE) {
                ...
                if ($rsElemento === TRUE) {
                    ...
                    if ($rsServicio === TRUE) {
                        ...
                        $estado = "NO SE PUDO CREAR EL SERVICIO [ ". $responseApi["message"]." ]";
                        // informacion para construir log de respuesta y error
                        $data = [
                            "messageLog"    => "ERROR: NO SE PUDO CREAR SERVICIO [ " . $responseApi["message"] . " ]",
                            "type"          => "SERVICIO",
                            "platform"      => "VO",
                            "state"         => FALSE,
                            "detail"        => "",
