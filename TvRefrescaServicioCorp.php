...
class TvRefrescaServicioCorp
{
    ...
    private function _setRefrescaServicioVo($vo, $info)
    {
        ...
        if ($rsPlan === TRUE) {
            ...
            if ($rsServicio === TRUE) {
                ...
                $estado = "NO SE PUDO ACTUALIZAR LOS PLANES DEL SERVICIO";
                // informacion para construir log de respuesta y error
                $data = [
                    "messageLog"    => "ERROR: NO SE PUDO ACTUALIZAR PLANES",
                    "type"          => "SERVICIO",
                    "platform"      => "VO",
                    "state"         => FALSE,
                    "detail"        => "",
    ...
    private function _validaPlanes()
    {
        ...
        if (
            !preg_match("/^'([0-2][0-9]|3[0-1])\/(0[1-9]|1[0-2])\/(\d{4})','(([0-2][0-9]|3[0-1])\/(0[1-9]|1[0-2])\/(\d{4}))*',(\d{1,4})(\|\d{1,4})*$/i", $this->_planes)
